...
abstract class Task implements _i1.TableRow<int?>, _i1.ProtocolSerialization {
  ...
  static TaskInclude include({_i2.TaskInclude? parentTask}) {
    return TaskInclude._(parentTask: parentTask);
  }
...
class TaskTable extends _i1.Table<int?> {
  ...
  _i2.TaskTable get parentTask {
    ...
    _parentTask = _i1.createRelationTable(
      relationFieldName: 'parentTask',
      field: Task.t.parentTaskId,
...
class TaskInclude extends _i1.IncludeObject {
  TaskInclude._({_i2.TaskInclude? parentTask}) {
    _parentTask = parentTask;
  }

  _i2.TaskInclude? _parentTask;

  @override
  Map<String, _i1.Include?> get includes => {'parentTask': _parentTask};
...
class TaskAttachRowRepository {
  ...
  /// Creates a relation between the given [Task] and [Task]
  /// by setting the [Task]'s foreign key `parentTaskId` to refer to the [Task].
  Future<void> parentTask(
    ...
    if (parentTask.id == null) {
      throw ArgumentError.notNull('parentTask.id');
    }
